...
**The** `filter( )` **function** is used to select which *rows* you would like to include. To select a single parameter, use an '=='. To select multiple parameters, use an '%in%', along with a vector of the parameters( in the format c(x1,x2,...)). Ex: filter(season == 5) or filter(season %in% c(5,6))
...
**The** `count( )` **function** is used to count up a number of objects. It can be used in place of the summarize() function for counting purposes. The argument sort = TRUE can be added to replace the arrange(desc()) function and argument as well. 
...
**Practice!**
Create a data set like the one above, but for season 2. Report a table including the following columns: season, episode, episode_name, and imdb_rating. Fill in the correct information in the *filter( )* and *select( )* rows.
...
## Practice using group_by( ) and count( )
...
Now, using what you've learned about the filter() function, create a data set with the average rating and number of episodes for seasons 3 and 8. Use the code in the example above as a reference.
...
Now take a look at this data graphic using the new **seasons_avg** dataset created!
